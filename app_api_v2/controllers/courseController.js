        ...
        Course.findById(req.user.pre_registered.course_id)
            ...
            .then(function(user) {
                ...
                return Course.update({
                    "_id": req.user.pre_registered.course_id,
                    "sections._id": req.user.pre_registered.section_id
                }, {
                    $set: {
                        [query_string]: "complete"
                    }
                });
            })
            .then(function(data) {
...
var joinCourse = function(req, res, next) {
    ...
    Course.findOne({
        'sections.section_key': req.body.section_key
    }, function(err, course) {
        if (err || !course) {
            return res.status(404).json({
                success: false,
...
var getUserCourses = function(req, res) {
    ...
    if (req.decodedToken.role === roles.STUDENT) {
        Course.find({
            'sections.students.student_id': req.decodedToken.sub
